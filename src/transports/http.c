...
static int apply_basic_credential(git_buf *buf, git_cred *cred)
{
	...
	if (git_buf_puts(buf, "Authorization: Basic ") < 0 ||
		base64_cred(buf, c->username, c->password) < 0 ||
		git_buf_puts(buf, "\r\n") < 0)
		return -1;
	...
	ssize_t content_length)
{
	...
	if (cred &&
		...
		apply_basic_credential(buf, cred);
...
static void clear_parser_state(http_subtransport *t)
{
	...
	memset(&t->parse_header_name, 0x0, sizeof(git_buf));

	git_buf_free(&t->parse_header_value);
	memset(&t->parse_header_value, 0x0, sizeof(git_buf));
	...
	memset(&t->www_authenticate, 0x0, sizeof(git_vector));
}

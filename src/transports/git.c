...
static int send_command(_git_stream *s)
{
	int error;
...
static int _git_stream_read(
	git_smart_subtransport_stream *stream,
	char *buffer,
	size_t buf_size,
	size_t *bytes_read)
{
	_git_stream *s = (_git_stream *)stream;
	gitno_buffer buf;
...
static void _git_stream_free(git_smart_subtransport_stream *stream)
{
	_git_stream *s = (_git_stream *)stream;
	...
	git_smart_subtransport_stream **stream)
{
	...
	s = (_git_stream *)git__calloc(sizeof(_git_stream), 1);
	GITERR_CHECK_ALLOC(s);

	s->parent.subtransport = &t->parent;
	s->parent.read = _git_stream_read;
	s->parent.write = _git_stream_write;
	s->parent.free = _git_stream_free;
	...
	git_smart_subtransport_stream **stream)
{
	...
	*stream = NULL;
	...
	if (_git_stream_alloc(t, url, cmd_uploadpack, stream) < 0)
		return -1;

	s = (_git_stream *)*stream;
